...
- `index.html` carries JSON-LD structured data (FAQPage, WebApplication), meta tags, and a `<noscript>` block. The `<noscript>` holds the full, themed, no-JavaScript eligibility rules, injected at build time by `scripts/inject-rules.mjs`. Source `index.html` keeps only a `<!--STATIC_RULES-->` placeholder. A second `<noscript><style>` in `<head>` hides the unhydrated `<ardc-footer>` when JavaScript is disabled (its fallback links would otherwise occupy blank layout height); it is inert when JavaScript is enabled, so do not remove it as redundant.
