...
**GA4 feature**: Explore > Free-form exploration. **Name**: `Step Back Analysis`. UX diagnostic that identifies which questions users reconsider.
...
| Session Trend             | Line: Date by Event count for `session_restored`.                                                        |
...
**GA4 feature**: Detail report. **Purpose**: assessment path by outcome breakdown with duration. **Default date range**: Last 28 days.
